...
class MotionDetector:
    ...
    def process_algo_per_frame(self, frame, capture, coordinates_data, times, statuses):
        ...
        if self.previousStatusesCount != self.currentStatusesCount:
            ...
            self.list.append(str(self.area.get_vehicle_status(zones)))
            if self.count == 13:
                #print(mode(self.list))
                self.persistVehicleStatusData(mode(self.list))
                self.count = 0
                self.list = []
                '''
    ...
    def persistVehicleStatusData(self, condition):        
        sql = ('insert into smart_traffic_control(area_1, precise_counter) ' +
               'values(:area_1, :precise_counter)')
        ...
        try:
            ...
            with cx_Oracle.connect('gsmuser/oracle@//localhost:1521/orcl') as connection:
                ...
                with connection.cursor() as cursor:
                    ...
                    if condition == 'VEHICLE:OFF':
                        cursor.execute(sql, ['OFF', time.time_ns()])
                    elif condition == 'VEHICLE:ON':
                        cursor.execute(sql, ['ON', time.time_ns()])
                    # commit work
                    connection.commit()
